...
describe('validateEnv — configuration refusée au démarrage', () => {
  ...
  it('accepte une configuration saine', () => {
    expect(
      validateEnv({ ...valid, TRUST_PROXY_HOPS: '0', AUTH_LOGIN_LIMIT: '10' }),
    ).toMatchObject(valid);
  });
  ...
  it('refuse une clé JWT de moins de 32 octets', () => {
    expect(() =>
      validateEnv({ ...valid, JWT_ACCESS_SECRET: 'trop-courte' }),
    ).toThrow(/trop court/);
  });
  ...
  it('refuse une limite anti-brute-force non numérique (sinon NaN la désactiverait)', () => {
    expect(() => validateEnv({ ...valid, AUTH_LOGIN_LIMIT: 'dix' })).toThrow(
      /AUTH_LOGIN_LIMIT/,
    );
    expect(() => validateEnv({ ...valid, AUTH_LOGIN_LIMIT: '0' })).toThrow(
      /AUTH_LOGIN_LIMIT/,
    );
  });
  ...
  it('refuse un nombre de sauts de proxy négatif ou décimal', () => {
    ...
    expect(() => validateEnv({ ...valid, TRUST_PROXY_HOPS: '1.5' })).toThrow(
      /TRUST_PROXY_HOPS/,
    );
    ...
    expect(() => validateEnv({ ...valid, TRUST_PROXY_HOPS: '3' })).toThrow(
      /TRUST_PROXY_HOPS/,
    );
  });
});
