...
void main() {
  ...
  setUp(() async {
    ...
    expect(await container.read(authControllerProvider.future), isA<AuthSignedIn>());
  });
  ...
  group('déconnecter TOUS les appareils (audit I3)', () {
    ...
    test('un succès ferme la session locale et le dit', () async {
      ...
      expect((state! as AuthSignedOut).message, contains('Toutes vos sessions'));
      expect(tokens.refresh, isNull);
    });
  });

  test('déconnexion simple : au mieux, même hors ligne', () async {
    api.logoutFailure = const ApiException(statusCode: 0, message: 'hors ligne');
  ...
  group('changement de mot de passe (revue C9)', () {
    ...
    test('un refus remonte à l’écran sans toucher à la session', () async {
      ...
      await expectLater(
        controller().changePassword(currentPassword: 'x', newPassword: 'Nouveau1234'),
