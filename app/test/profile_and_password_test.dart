  ...
  await tester.pumpWidget(
    ProviderScope(
      overrides: [
        ...
        pendingMutationsCountProvider.overrideWith(
...
void main() {
  group('Mon profil', () {
    testWidgets(
      '« Tout déconnecter » qui échoue : dit que la révocation n’a PAS eu lieu (I3)',
      (tester) async {
        final (api, tokens) = await _pump(
          tester,
    ...
    testWidgets(
      'se déconnecter avec des opérations en attente : prévient d’abord (I2)',
      (tester) async {
    ...
    testWidgets('sans opération en attente : aucun dialogue superflu (§12.7)', (
      tester,
    ) async {
    ...
    testWidgets(
      'signale les opérations laissées par un AUTRE compte sur l’appareil',
      (tester) async {
        ...
        expect(
          find.textContaining('jamais avec votre session'),
          findsOneWidget,
        );
      },
    );
  ...
  group('Changement de mot de passe', () {
    testWidgets(
      'mauvais mot de passe ACTUEL : message exact, saisie conservée (C9)',
      (tester) async {
        final (api, _) = await _pump(
          tester,
    ...
    testWidgets('les champs de mot de passe ne sont ni corrigés ni suggérés', (
      tester,
    ) async {
