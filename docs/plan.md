...
21. Rapports (ventes, stock, achats) + **exports Excel/CSV & PDF d'historique** _(2026-09-26, **tranche A** : les TROIS rapports de lecture livrés et prouvés (ventes avec marge et tendance, stock valorisé au coût, achats commandé/reçu), ADMIN seul. **Tranche B à faire : les exports Excel/CSV et PDF** — `exceljs` n'est pas encore installé, `common/export/` reste à créer ; `common/pdf/pdf.ts` existe déjà)_
