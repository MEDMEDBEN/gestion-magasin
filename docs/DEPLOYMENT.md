...
4. Copier `infra/.env.example` → `infra/.env`, remplir toutes les valeurs (mots de passe, clés JWT générées aléatoirement). **`STORE_NAME`, `STORE_NIF`, `STORE_RC`** (et NIS/AI/adresse/téléphone) : sans NIF ni RC, les factures PDF sont refusées.
